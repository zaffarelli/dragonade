...
def card_reveal(request):
    ...
    return render(request, 'main/card_reveal.html', context=context)
...
def draconis_artes(request):
    ...
    context['config']['menu_entries'] = MENU_ENTRIES
    spells = []
    for i in Spell.objects.all().order_by("diff"):
        spells.append(i.export_to_json())
    print(spells)
    context['spells'] = spells
    return render(request, 'main/draconis_artes.html', context=context)
